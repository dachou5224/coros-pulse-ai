...
last_8_weeks = df.iloc[1:9]

# --- 核心指标看板 (KPIs) ---
st.subheader(f"📅 本周概览 ({latest_week['Week Start'].strftime('%m-%d')} ~ {latest_week['Week End'].strftime('%m-%d')})")
...
with col4:
    tsb_val = latest_week['Form (TSB)']
    col4.metric("Form (TSB)", tsb_val, delta=None)
with col5:
    decouple = latest_week['LSD Decouple']
    display_metric(col5, "有氧解耦率", decouple, 'LSD Decouple', last_4_weeks, "%", reverse_color=True)
...
with c2:
    ...
    fig_tsb = go.Bar(
        x=chart_df['Week Start'],
        y=chart_df['Form (TSB)'],
        marker_color=chart_df['Form (TSB)'].apply(lambda x: '#10B981' if x >= 0 else ('#EF4444' if x < -20 else '#F59E0B'))
    )
    layout_tsb = go.Layout(
        yaxis=dict(title='TSB'),
        shapes=[
            dict(type="line", x0=chart_df['Week Start'].min(), x1=chart_df['Week Start'].max(), y0=0, y1=0, line=dict(color="gray", dash="dash")),
            dict(type="line", x0=chart_df['Week Start'].min(), x1=chart_df['Week Start'].max(), y0=-20, y1=-20, line=dict(color="red", dash="dot")),
        ]
    )
    fig2 = go.Figure(data=fig_tsb, layout=layout_tsb)
...
with col_p1:
    ...
    st.plotly_chart(fig_vdot, use_container_width=True)

with col_p2:
    fig_dec = px.area(chart_df, x='Week Start', y='LSD Decouple', title='LSD 有氧解耦率 (越低越稳)', markers=True)
    fig_dec.add_hline(y=5, line_dash="dash", line_color="red", annotation_text="5% 警戒线")
    st.plotly_chart(fig_dec, use_container_width=True)

# --- 原始报表 ---
with st.expander("📊 查看历史周报数据"):
    display_cols = ['Week Start', 'Week End', 'Distance (km)', 'Weekly Load', 'Fitness (CTL)', 'Form (TSB)', 'LSD Decouple', 'Status']
